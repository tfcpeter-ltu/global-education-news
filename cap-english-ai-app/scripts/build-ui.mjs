...
for(const file of files){
  const r=await fetch(base+file+'?v=20260907-clean');
  if(!r.ok) throw new Error(`Failed to fetch ${file}: ${r.status}`);
  await writeFile(`public/${file}`,await r.text(),'utf8');
  console.log('copied',file);
}
const index=`<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>2027 國中教育會考英文 AI｜LTU AI English</title><meta name="description" content="用 AI 找出英文弱點，不靠盲目刷題。個人化錯因診斷、會考模擬、AI 重點筆記與長期弱點追蹤。"><style>html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#fff}iframe{border:0;width:100%;height:100%;display:block}</style></head><body><iframe src="/final-v18.html?v=20260907-clean" title="2027 會考英文 AI" allow="microphone"></iframe></body></html>`;
await writeFile('public/index.html',index,'utf8');
console.log('CAP English AI clean V18 UI ready');
