...
const cache='20260913-feature-v22-tradtw';
...
let encoded=fparts.join(''),feature=null;
for(let layer=0;layer<3;layer++){
  const b=Buffer.from(encoded,'base64');
  if(b.length>16 && b.subarray(4,12).toString('ascii')==='ftypavif'){feature=b;break}
  const s=b.toString('utf8').replace(/\s+/g,'');
  if(!/^[A-Za-z0-9+/=]+$/.test(s) || s.length<1000)break;
  encoded=s;
}
if(!feature) throw new Error('Feature image decode failed');
await writeFile('public/cap-feature-hero-v22.avif',feature);
...
console.log(`CAP English AI built: Traditional Chinese UI + feature AVIF ${feature.length} bytes`);
