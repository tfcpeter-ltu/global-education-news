...
for(const file of files){
  const r=await fetch(base+file+'?v=20260907-approved-art');
  if(!r.ok) throw new Error(`Failed to fetch ${file}: ${r.status}`);
  await writeFile(`public/${file}`,await r.text(),'utf8');
  console.log('copied',file);
}
await writeFile('public/ltu-refine-v2.js',await readFile('ltu-refine-v2.js','utf8'),'utf8');
const index=`<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>2027 國中教育會考英文 AI｜LTU 國際學術中心</title><meta name="description" content="LTU 國際學術中心系列 AI 輔助教材。免費會員每日 3 題；完整會員提供 AI 弱點記憶、每週會考模考、AI 筆記與能力分析。"><style>html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#fff}iframe{border:0;width:100%;height:100%;display:block}</style></head><body><iframe src="/final-v19.html?v=20260907-approved-art" title="2027 會考英文 AI" allow="microphone"></iframe><script src="/ltu-refine-v2.js?v=20260907-approved-art"></script></body></html>`;
await writeFile('public/index.html',index,'utf8');
console.log('CAP English AI approved artwork homepage ready');